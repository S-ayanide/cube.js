...
Often at the beginning of an analytical system's lifecycle - when there is a
smaller dataset that queries execute over - the system works well and delivers
responses within acceptable thresholds. However, as the size of the dataset
grows, the time-to-response from a user's perspective can often suffer quite
heavily. This is true of both application and purpose-built data warehousing
solutions.
...
for number of orders completed per day, and that this query is performing
...
In order to add improve the performance of this query, we can add another
